								 ...
								 uint32_t domain, g_string name, const string& prefix, uint32_t tBL, double timing_scale) 
{
    ...
    bool ifhbm = config.get<bool>(prefix + "ifhbm", false);

    auto mem = (DDRMemory *) gm_malloc(sizeof(DDRMemory));
	new (mem) DDRMemory(zinfo->lineSize, pageSize, ranksPerChannel, banksPerRank, frequency, tech, addrMapping, controllerLatency, queueDepth, maxRowHits, deferWrites, closedPage, domain, name, ifhbm, tBL, timing_scale);
    return mem;
}
